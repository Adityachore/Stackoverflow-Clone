        ...
        <Mainlayout>
            <div className="max-w-6xl mx-auto p-6">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    {products.map((p, i) => (
                        <div key={i} className="p-8 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition card-hover block">
                            <div className="mb-4">{p.icon}</div>
                            <h3 className="text-xl font-bold text-gray-900 mb-2">{p.title}</h3>
                            <p className="text-gray-600 leading-relaxed">{p.description}</p>
                            <button className="mt-4 text-sm font-semibold text-blue-600 hover:text-blue-800">Learn more →</button>
                        </div>
                    ))}
                </div>
            </div>
